...
test("Get Ship Coordinates on Empty Board", () => {
  ...
  expect(GameModule.getValidBoardLines("E5", 5)).toEqual([
    ["E5", "E9"],
    ["E5", "E1"],
    ["E5", "I5"],
    ["E5", "A5"],
  ...
  expect(GameModule.getValidBoardLines("A1", 3)).toEqual([
    ["A1", "A3"],
    ["A1", "C1"],
  ]);

  //Only three valid placements on an empty board
  expect(GameModule.getValidBoardLines("B8", 3)).toEqual([
    ["B8", "B10"],
    ["B8", "B6"],
    ["B8", "D8"],
  ]);
...
test("CPU Places Random Ships", () => {
  ...
  expect(match.humanPlayer.gameBoard.shipsOnBoard.length).toBe(0);
...
test("AI Attack", () => {
  ...
  expect(
    match.humanPlayer.gameBoard.successfulAttacks.length > 0 ||
      match.humanPlayer.gameBoard.missedAttacks.length > 0
  ).toBeTruthy();
});
