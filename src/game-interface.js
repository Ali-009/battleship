...
function displayGameBoard() {
  ...
  startButton.addEventListener("click", () => {
    ...
    const submitPlayerShip = function (shipName, shipLength) {
      let message = document.querySelector(".message");
      let shipValidity = checkCoordinatesValidity(
        [startPointField.value, endPointField.value],
        shipLength
      );
      ...
      try {
        match.humanPlayer.gameBoard.placeShip(shipName, [
    ...
    function coordinateSubmitListener() {
      if (shipCount < 5) {
        let submittedShip = submitPlayerShip(
          shipNames[shipCount],
          shipLengths[shipCount]
        );
        if (submittedShip) {
          shipCount++;
          if (shipCount < 5) {
          ...
          coordinateSubmit.removeEventListener(
            "click",
            coordinateSubmitListener
          );
      ...
      if (shipCount === 5) {
        ...
        message.textContent =
          "You can now attack the AI's ships on the trakcing gird. Note that a player geting a hit gives them an additional turn.";
        //Setup the AI ships
...
function generateGrid(grid) {
  for (let i = 0; i < 100; i++) {
    ...
    gridCell.setAttribute(
      "data-cell",
      `${columnNotation[i % 10] + (Math.floor(i / 10) + 1)}`
    );
